...
def download_chlorophyll_conc(date,output_dir):
    """
    ...
    """
    # Build file path
    url = "https://www.ocims.gov.za/data/s3olci/s3-chl-south_africa"
    file_in = os.path.join(url, f"chl_south_africa_{date.strftime('%Y%m%d')}.tif" )
    try:
        print(f"\nOpening tif file: {file_in}")
        da = rioxarray.open_rasterio(file_in)
        
        print("\nConverting to xarray dataset...")
        ds = da.to_dataset(name="chl")

        file_out = os.path.join(output_dir, f"CHL_{date.strftime('%Y%m%d_%H')}.nc")
        print(f"\nWriting NetCDF to: {file_out}")
        ds.to_netcdf(file_out)
...
if __name__ == '__main__':
    date = pd.to_datetime('2026-05-14 00:00:00')
    output_dir = '/home/g.rautenbach/Downloads/'
    download_chlorophyll_conc(date, output_dir)
